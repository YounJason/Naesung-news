(function () {
    ...
    function setOgTags(meta) {
        const head = document.head;

        function addTag(property, content) {
            if (!content) return;
        ...
        addTag("og:title", meta.title || "내성 신문");
        addTag("og:description", `${meta.author || ""} ${meta.date || ""}`);
        addTag("og:type", "article");
        addTag("og:url", window.location.href);
        addTag("og:image", meta.image || "/default.jpg");
    }
    ...
    (async function run() {
        ...
        try {
            ...
            render({
                ...
                html: `<p>요청한 파일을 찾지 못했어요. ${err.tried ? "시도한 경로: <br>" + err.tried.map(escHTML).join("<br>") : ""
                    }</p>`
            });
        }
