...
const CONTENTS = [
  ...
  { dir: "sports", json: "sports.json", out: "sv" },
];
...
async function main() {
  ...
  // 3) 각 카테고리 빌드
  for (const { dir, json, out } of CONTENTS) {
    const idxBySlug = loadIndexMap(json);
    ...
    for (const file of files) {
      ...
      const url = `${SITE_ORIGIN.replace(/\/$/, "")}/${out}/${encodeURIComponent(slug)}`;
      ...
      const outDir = path.join(DIST, out, slug);
      await fs.ensureDir(outDir);
      await fs.writeFile(path.join(outDir, "index.html"), html, "utf8");
    }
  }

  console.log("✅ OG 데이터(articles.json, sports.json 우선) 반영 완료.");
}
